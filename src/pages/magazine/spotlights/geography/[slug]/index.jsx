...
export default function SpotlightsGeography({ spotlight, ...props }) {
  return (
    <>
      <Head>
...
export async function getStaticPaths() {
  ...
  const paths = spotlights.geographies.map((geography) => ({
    params: { slug: geography.toLowerCase().trim().replace(/[^\w\s-]/g, '').replace(/[\s_-]+/g, '-').replace(/^-+|-+$/g, '') },
  }))
