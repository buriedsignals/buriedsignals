...
export default function SpotlightsAward({ spotlight, ...props }) {
  return (
    <>
      <Head>
...
export async function getStaticPaths() {
  ...
  const paths = spotlights.awards.map((award) => ({
    params: { slug: award.toLowerCase().trim().replace(/[^\w\s-]/g, '').replace(/[\s_-]+/g, '-').replace(/^-+|-+$/g, '') },
  }))
